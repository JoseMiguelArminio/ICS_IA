...
export const getActorById = (id) => ACTORS.find(actor => actor.id === id);

export const getCastByMovieId = (movieId) => {
  return ROLES.filter(role => role.movieId === movieId)
              .map(role => ({ ...getActorById(role.actorId), character: role.character }));
};

export const getFilmographyByActorId = (actorId) => {
  return ROLES.filter(role => role.actorId === actorId)
              .map(role => getMovieById(role.movieId));
};
